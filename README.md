...
```bash
$ w4f --target api.example.bank --target api.bank-example.co.id --timeout 6
```
...
cert      SSL Corporation
  san     api.bank-example.co.id, www.api.bank-example.co.id
...
cert      example-registrar.com, Inc.
  san     api.example.bank, www.api.example.bank
...
verdict   cloudflare (7): header server: cloudflare; header cf-ray: ...;
          cookie: _cfuvid=...; cname: api.example.bank.cdn.cloudflare.net;
