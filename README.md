...
- **`aws-ec2`** — bare EC2 origins (PTR `compute-N.amazonaws.com`) now get
  a verdict instead of "unknown edge" (`analytics.example.com`).
- **HAProxy cookie** — stick-table persistence cookie `<name>=!<base64>`
  (`mbanking.example.co.id`'s `brks_lb=!…`), for hosts that hide the
  Server header.
- **`gtm-gslb`** — DNS GTM/GSLB CNAMEs `gtm-<region>-<hash>.gtm-i1d6.com`
  (`api-external.bank-example.co.id`); classified as the load-balancer class, not
...
- **header read on untrusted chains** — `http_get` now retries without
  ...
  private-chain hosts still expose their headers (`bank-example.co.id` →
